...
from app.schemas.auth import PerformPasswordReset, RequestPasswordReset, ValidateResetPasswordToken
...
def signup_route(user_in: UserCreate, session: Session = Depends(get_session)):
...
def request_password_reset_route(
    data: RequestPasswordReset, background_tasks: BackgroundTasks, session: Session = Depends(get_session)
):
...
def confirm_password_reset_route(
    request: Request, data: PerformPasswordReset, session: Session = Depends(get_session)
):
...
def validate_reset_password_token_route(
    request: Request, data: ValidateResetPasswordToken, session: Session = Depends(get_session)
):
